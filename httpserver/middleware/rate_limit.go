...
func (h *rateLimitHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	requestHandler := &rateLimitRequestHandler{rw: rw, r: r, next: h.next, parent: h}
...
func (h *rateLimitRequestHandler) OnReject(params ratelimit.Params) error {
	h.parent.onReject(h.rw, h.r, h.convertParams(params), h.next, GetLoggerFromContext(h.r.Context()))
	return nil
}

func (h *rateLimitRequestHandler) OnRejectInDryRun(params ratelimit.Params) error {
	// HTTP middleware doesn't differentiate between regular reject and dry run reject.
	// The dry run behavior is handled in makeRateLimitOnRejectFunc.
	return h.OnReject(params)
}

func (h *rateLimitRequestHandler) OnError(params ratelimit.Params, err error) error {
	h.parent.onError(h.rw, h.r, h.convertParams(params), err, h.next, GetLoggerFromContext(h.r.Context()))
	return nil
}

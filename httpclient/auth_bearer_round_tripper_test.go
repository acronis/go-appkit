...
func (s *AuthBearerRoundTripperTestSuite) TestRoundTrip() {
	...
	s.Require().Equal(fmt.Sprintf("Bearer %s", authProvider.token), resp.Header.Get(reqAuthorizationHeader))
...
func (s *AuthBearerRoundTripperTestSuite) TestCacheInvalidationOn401() {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		switch authHeader {
		case fmt.Sprintf("Bearer %s", oldToken):
			rw.WriteHeader(http.StatusUnauthorized)
		case fmt.Sprintf("Bearer %s", newToken):
	...
	s.Require().Equal(fmt.Sprintf("Bearer %s", newToken), resp.Header.Get(reqAuthorizationHeader))
